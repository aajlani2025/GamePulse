...
async function updateRefreshHashById(id, refreshHash) {
  const sql = `UPDATE users SET refresh_token_hash = $2, last_login_at = NOW() WHERE id = $1`;
  await pool.query(sql, [id, refreshHash]);
}

async function updateRefreshHashByUsername(username, refreshHash) {
  const sql = `UPDATE users SET refresh_token_hash = $2 WHERE username = $1 RETURNING id`;
...
async function findByUsername(username) {
  ...
  const sql = `SELECT id, username, password_hash, refresh_token_hash FROM users WHERE username = $1 LIMIT 1`;
